...
def simulate_xrd_to_hdf5(
    ...
    kwargs = {
        "bkg_-1": (0.0, 0.03),   # much smaller slope
        "bkg_-2": (0.0, 0.05),
        "sample_height": (-0.1, 0.1),
    }
    ...
    if allowed_ids is None:
        ...
        total_cifs = sum(
            ...
            if f.endswith(".cif") and (
                f.replace(".cif", "") in allowed_ids or
                f.replace(".cif", "").split("_")[0] in allowed_ids
            )
        )

        if max_cifs is not None:
            total_cifs = min(total_cifs, max_cifs)
    ...
    with h5py.File(out_file, "w") as f:
        ...
        dset_xrd = f.create_dataset(
            "xrd", (0, num_points), maxshape=(None, num_points),
        ...
        for source in base_dirs:
            for fname in sorted(os.listdir(source)):
                ...
                base_id = file_id.split("_")[0]  # handles mp-1234_SG62
         
            
                if allowed_ids is not None:
                    if file_id not in allowed_ids and base_id not in allowed_ids:
                        continue



                cif_seen += 1
                print(f"\n🔹 Processing CIF: {fname}  ({cif_seen}/{total_cifs})")
                if max_cifs is not None and cif_seen > max_cifs:
                    break
                ...
                try:
                    ...
                    for i in range(n_patterns_per_cif):
                        ...
                        hkls = [
                            tuple(peak[0]["hkl"]) if peak and "hkl" in peak[0] else (0, 0, 1)
                            for peak in pattern.hkls
                        ]
                        print(f"   Diffraction computed ({len(two_theta)} peaks)")
                        print(f"   Augmenting {n_patterns_per_cif} patterns...")
                        xrd_curve = augment_xrd_pattern(
                            two_theta, intensity, hkls,
                            structure, x_uniform, rng, param_dict, kwargs
                        )

                        batch_xrd.append(xrd_curve.astype("float32"))
                        batch_labels.append(sg_num)
                        batch_ids.append(f"{fname}_aug{i}")
                        ...
                        if (index + 1) % print_every == 0:
                            dt = time.time() - t0
                            rate = (index + 1) / dt if dt > 0 else 0
                            approx_total_patterns = total_cifs * n_patterns_per_cif
                            print(
                                f"[{index+1:,}/{approx_total_patterns:,}] "
                                f"{rate:.1f} patterns/s | last: {fname}"
                            )
